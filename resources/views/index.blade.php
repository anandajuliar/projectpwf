    ...
    <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <form>
            <div class="mb-4">
                ...
                <input type="text" class="w-full p-3 border rounded-xl" placeholder="Misal: Chef Juna" required>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-bold text-gray-700 mb-2">Email Login</label>
                <input type="email" class="w-full p-3 border rounded-xl" placeholder="juna@pwf.com" required>
            </div>
            <div class="mb-6">
                <label class="block text-sm font-bold text-gray-700 mb-2">Password Sementara</label>
                <input type="password" class="w-full p-3 border rounded-xl" placeholder="••••••••" required>
            </div>
            
            <div class="flex justify-end space-x-4">
                <button type="button" onclick="tutupModalChef()" class="px-5 py-2 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200">Batal</button>
                <button type="button" onclick="alert('Data pendaftaran siap ditembak ke /api/auth/register !')" class="px-5 py-2 bg-amber-600 text-white font-bold rounded-xl hover:bg-amber-700">Buat Akun</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function bukaModalChef() { document.getElementById('modalTambahChef').classList.remove('hidden'); }
    function tutupModalChef() { document.getElementById('modalTambahChef').classList.add('hidden'); }
</script>
@endsection